...
const getAllBuildsFromGame = async (request, response) => {
  try {
    const builds = await DS1BuildModel.find({}).populate('user', { username: 1, name: 1 });
...
const getUserBuildsFromGame = async (request, response) => {
  try {
    if (request.token) {
      const builds = await DS1BuildModel.find({ user: request.user.id });
      return response.json(builds);
    } else if (request.anonymousUserId) {
      const builds = await DS1BuildModel.find({ anonymousUserId: request.anonymousUserId });
      return response.json(builds);
    }
...
const getBuildById = async (request, response) => {
  try {
    const build = await DS1BuildModel.findById(request.params.id).populate('user', { username: 1, name: 1 });
    if (build) {
      response.json(build);
    } else {
...
const postBuild = async (request, response) => {
  try {
    const buildToSave = new DS1BuildModel(request.body);
...
const updateBuildById = async (request, response) => {
  try {
    const buildToUpdate = await DS1BuildModel.findById(request.params.id);
    ...
    const updatedBuild = await DS1BuildModel.findByIdAndUpdate(request.params.id, newBuild, {
      new: true,
      runValidators: true,
      context: 'query'
    });
...
const deleteBuildById = async (request, response) => {
  try {
    const buildToDelete = await DS1BuildModel.findById(request.params.id);
    ...
    await DS1BuildModel.findByIdAndDelete(request.params.id);
