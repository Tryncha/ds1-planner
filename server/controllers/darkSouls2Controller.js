...
const getAllBuildsFromGame = async (request, response) => {
  try {
    const builds = await DS2BuildModel.find({}).populate('user', { username: 1, name: 1 });
...
const getUserBuildsFromGame = async (request, response) => {
  try {
    if (request.token) {
      const builds = await DS2BuildModel.find({ user: request.user.id });
      return response.json(builds);
    } else if (request.anonymousUserId) {
      const builds = await DS2BuildModel.find({ anonymousUserId: request.anonymousUserId });
      return response.json(builds);
    }
...
const getBuildById = async (request, response) => {
  try {
    const build = await DS2BuildModel.findById(request.params.id).populate('user', { username: 1, name: 1 });
    if (build) {
      response.json(build);
    } else {
...
const postBuild = async (request, response) => {
  try {
    const buildToSave = new DS2BuildModel(request.body);
    ...
    if (request.user) {
      ...
      request.user.builds.darkSouls2 = request.user.builds.darkSouls2.concat(savedBuild._id);
...
const updateBuildById = async (request, response) => {
  try {
    const buildToUpdate = await DS2BuildModel.findById(request.params.id);
    ...
    const updatedBuild = await DS2BuildModel.findByIdAndUpdate(request.params.id, newBuild, {
      new: true,
      runValidators: true,
      context: 'query'
    });
...
const deleteBuildById = async (request, response) => {
  try {
    const buildToDelete = await DS2BuildModel.findById(request.params.id);
    ...
    if (request.user) {
      request.user.builds.darkSouls2 = request.user.builds.darkSouls2.filter(
        (buildId) => buildId.toString() !== request.params.id
      );
      await request.user.save();
    }

    await DS2BuildModel.findByIdAndDelete(request.params.id);
