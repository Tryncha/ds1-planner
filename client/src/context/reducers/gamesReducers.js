...
async function fetchStartingClasses() {
  const response = await axios.get('/api/data/dark-souls-1/starting-classes');
  console.log('API:', response.data);
}

fetchStartingClasses();

export function darkSouls1Reducer(buildState, action) {
  switch (action.type) {
    case 'SET_TITLE':
      return {
        ...buildState,
